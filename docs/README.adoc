...
+
-----
echo 'select cm.PrunedCourseId, cm.CourseId, c.CourseName from CourseMappings cm, Courses c where cm.courseId = c.courseId' | mysql -u root -p -B lms_transactional > /tmp/Courses_\&_Mappings.tsv
-----
