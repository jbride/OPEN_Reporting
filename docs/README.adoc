...
== GPTE Reporting Shortcuts

Each environment contains shortcuts to tail logs and bounce services.

. *ta* :   Tails the accreditation log
. *ba* :   Bounces the accreditation services
. *tg* :   Tails the GPTE log
. *bg* :   Bounces the GPTE services
...
.. `dashbuilder` contains data sources to lms_reporting, and uses data from lms_reporting to populate the online reports.
