...
. Wait for anywhere between 5 - 45 seconds.
...
NOTE:  If after having sent the course completion email with attachment, nothing related to the processing of this course completion email appears to have occurred after about 45 seconds or so, the GPTE Server may need to be bounced.  The camel mail component used in GPTE Reporting) may have gone stale.  This rarely happens however.  The same functionality found in production has run for months without having to be bounced.
