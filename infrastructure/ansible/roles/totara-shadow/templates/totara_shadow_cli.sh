...
if (outcome != success) of /subsystem=datasources/data-source=totaraShadowDS:read-attribute(name=enabled)
  # totaraShadowDS
  data-source add --name=totaraShadowDS --jndi-name="java:jboss/datasources/totaraShadowDS" --driver-name=postgresql --user-name={{totara_shadow_db_username}} --password={{totara_shadow_db_password}} --exception-sorter-class-name=org.jboss.jca.adapters.jdbc.extensions.postgres.PostgreSQLExceptionSorter --valid-connection-checker-class-name=org.jboss.jca.adapters.jdbc.extensions.postgres.PostgreSQLValidConnectionChecker --connection-url="jdbc:postgresql://{{totara_shadow_host_ip}}:{{totara_shadow_host_port}}/{{totara_shadow_db_name}}?sslmode=require&amp;sslfactory=org.postgresql.ssl.NonValidatingFactory"
